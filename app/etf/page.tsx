...
export default function EtfPage() {
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><div className="mx-auto max-w-6xl p-5 pb-12">
    ...
    <div className="grid gap-3">{data.analyses.map((item, index) => <article key={item.master.symbol} className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-800 font-black">{index + 1}</div><div><div className="font-bold">{item.master.symbol} {item.master.shortName}</div><div className="mt-1 text-xs text-slate-500">{item.master.strategy} · {item.master.issuer}</div></div></div><div className="text-right"><div className="text-3xl font-black text-emerald-300">{item.score.toFixed(1)}</div><div className={`text-xs font-bold ${signalClass(item.signal)}`}>{item.signal}</div></div></div><div className="mt-4 grid grid-cols-5 gap-2 text-xs"><div className="rounded-lg bg-slate-950 p-2"><div className="text-slate-500">価格</div><div className="mt-1 font-bold">¥{item.metrics.price.toLocaleString()}</div></div><div className="rounded-lg bg-slate-950 p-2"><div className="text-slate-500">7日</div><div className="mt-1 font-bold">{signed(item.metrics.return7d)}</div></div><div className="rounded-lg bg-slate-950 p-2"><div className="text-slate-500">20日</div><div className="mt-1 font-bold">{signed(item.metrics.return20d)}</div></div><div className="rounded-lg bg-slate-950 p-2"><div className="text-slate-500">60日</div><div className="mt-1 font-bold">{signed(item.metrics.return60d)}</div></div><div className="rounded-lg bg-slate-950 p-2"><div className="text-slate-500">Exit</div><div className="mt-1 font-bold">{item.exitScore.toFixed(1)}</div></div></div><div className="mt-3 text-xs text-slate-400">{item.reasons.join(" / ")}</div></article>)}</div>
