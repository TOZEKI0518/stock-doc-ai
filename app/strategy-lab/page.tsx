...
export default function StrategyLabPage() {
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><div className="mx-auto max-w-6xl p-4 pb-24 sm:p-6">
    <header className="mb-5"><Link href="/stocks" className="text-sm text-emerald-300 underline">← 株ドックAIへ戻る</Link><div className="mt-4 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">StockDoc AI Pro</p><h1 className="mt-1 text-3xl font-bold">Strategy Lab</h1><p className="mt-2 max-w-2xl text-sm text-slate-300">保存済みSnapshotだけを使い、AI予測の投資戦略を比較・検証します。AIモデルとSnapshot保存処理は変更しません。</p></div><div className="text-right text-xs text-slate-400"><div>最新Snapshot: {data?.latestSnapshotDate ?? "-"}</div><div>分析対象: {data?.tradeCount.toLocaleString() ?? 0}件</div></div></div></header>
